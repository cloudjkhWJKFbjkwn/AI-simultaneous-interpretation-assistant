...
- `src/components/SubtitleItem.tsx` — 单条字幕渲染（标记、修正动画、单词点击）
- `src/components/WordPopover.tsx` — 单词释义弹窗组件
- `src/services/MockTranslationService.ts` — 导出词典 + `lookupWord` 静态方法
...
- 折叠/展开动画流畅（300ms）
- 新字幕淡入、修正高亮闪烁动画不卡顿
- 点击英文单词弹出气泡显示中文释义，弹窗定位在单词附近
- 点击弹窗外或按 Escape 关闭弹窗，点击另一个单词切换弹窗内容
- MockTranslationService 导出的 `lookupWord` 方法正确返回释义或 null
- 手动滚动时不自动跳底，滚回底部时恢复
- 移动端布局正常切换到横条模式
